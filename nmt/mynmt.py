...
def init_config():
    ...
    parser.add_argument('--mode', choices=['train', 'att', 'raml_train', 'test', 'sample', 'prob', 'interactive'],
    ...
    parser.add_argument('--eval_dir', type=str, help='path to the eval file')

    parser.add_argument('--decode_max_time_step', default=50, type=int, help='maximum number of time steps used '
                                                                              'in decoding and sampling')

    parser.add_argument('--valid_niter', default=50, type=int, help='every n iterations to perform validation')
    ...
    parser.add_argument('--patience', default=50, type=int, help='training patience')
    ...
    parser.add_argument('--lr_decay', default=0.0, type=float, help='decay learning rate if the validation performance drops')
    parser.add_argument('--device0',default='cpu',type=str,help='used to decide gpu or cpu while storing tensors')
    parser.add_argument('--device1',default='cpu',type=str,help='used to decide gpu or cpu while storing tensors')
    # raml training
    ...
    print(args.device0,args.device1)
    return args
...
class NMT(nn.Module):
    ...
    def forward(self, args,src_sents, src_sents_len, tgt_words):
        src_encodings, init_ctx_vec = self.encode(args,src_sents, src_sents_len)
        scores, att_sims = self.decode(args,src_encodings, init_ctx_vec, tgt_words)

        return scores, att_sims

    def encode(self, args,src_sents, src_sents_len):
        """
        ...
        output, (_, last_cell) = self.encoder_lstm(packed_src_embed)

        output, _ = pad_packed_sequence(output)

        dec_init_cell = self.decoder_cell_init(torch.cat([last_cell[0], last_cell[1]], 1)).to(args.device1)
        dec_init_state = torch.tanh(dec_init_cell).to(args.device1)

        return output, (dec_init_state, dec_init_cell)

    def decode(self,args, src_encoding, dec_init_vec, tgt_sents):
        """
        ...
        :return:
        """
        init_state = dec_init_vec[0].to(args.device0)
        init_cell = dec_init_vec[1].to(args.device0)
        hidden = (init_state, init_cell)
        del dec_init_vec # modified
        ...
        src_encoding_att_linear = tensor_transform(self.att_src_linear, src_encoding).to(args.device0) # modified

        # initialize attentional vector
        att_tm1 = new_tensor(batch_size, self.args.hidden_size).zero_() # modified
        att_tm = att_tm1.to(args.device1)
        del att_tm1
        # print(self.tgt_embed.device,tgt_sents.device)
        tgt_word_embed = self.tgt_embed(tgt_sents).to(args.device1) # modified
        # if args.cuda:
        ...
        for y_tm1_embed in tgt_word_embed.split(split_size=1):
            ...
            # print(att_tm.device)
            x = torch.cat([y_tm1_embed.squeeze(0), att_tm,], 1).to(args.device0)
            ...
            score_t = self.readout(att_t).to(args.device1)   # E.q. (6)
            scores.append(score_t)
            att_sim_mat = att_sim_mat.to(args.device1) # modified
            att_sim_mats.append(att_sim_mat)
            att_tm = att_t.to(args.device1)
            hidden = h_t, cell_t
            del x,att_t,ctx_t,alpha_t,h_t,cell_t,att_sim_mat # modified
            torch.cuda.empty_cache()
            t += 1

        scores = torch.stack(scores).to(args.device1) # modified
        return scores, att_sim_mats

    def translate(self,args, src_sents, beam_size=None, to_word=True):
        """
        perform beam search
        ...
        src_sents_var = to_input_variable(src_sents, self.vocab.src, cuda=args.cuda, is_test=True,device=args.device0)
        # src_sents_var = to_input_variable(src_sents, self.vocab.src, cuda=False, is_test=True)

        src_encoding, dec_init_vec = self.encode(args,src_sents_var, [len(src_sents[0])])
        ...
        with torch.no_grad():
            ...
            if args.cuda:
                att_tm1 = att_tm1.to(args.device1)
                hyp_scores = hyp_scores.to(args.device1)
            ...
            while len(completed_hypotheses) < beam_size and t < args.decode_max_time_step:
                ...
                if args.cuda:
                    y_tm1 = y_tm1.to(args.device0)
                # print(y_tm1.device)
                att_tm1 = att_tm1.to(args.device0)
                ...
                score_t = self.readout(att_t).to(args.device1)
                ...
                if args.cuda:
                    live_hyp_ids = live_hyp_ids.to(args.device0)
...
def to_input_variable(sents, vocab, cuda=False,device='cpu', is_test=False):
    """
...
def evaluate_loss(args,model, data, crit):
    ...
    for src_sents, tgt_sents in data_iter(data, batch_size=args.batch_size, shuffle=False):
        ...
        src_sents_var = to_input_variable(src_sents, model.vocab.src, cuda=args.cuda,device=args.device0, is_test=True)
        tgt_sents_var = to_input_variable(tgt_sents, model.vocab.tgt, cuda=args.cuda,device=args.device0, is_test=True)

        # (tgt_sent_len, batch_size, tgt_vocab_size)
        scores, _ = model(args,src_sents_var, src_sents_len, tgt_sents_var[:-1])
        scores = scores.to('cuda:1')
        tgt_sents_var = tgt_sents_var.to('cuda:1')
...
def init_training(args):
    if args.load_model:
        ...
        # for i in range(100):
        #     print(vocab.tgt.id2word[i])
        # tg = vocab.tgt
        # print(tg.id2word[3],tg.id2word[4],tg.id2word[5])
    ...
    # if args.cuda:
    model = model.to(args.device0)
    # model.src_embed = model.src_embed.cpu()
    # model.tgt_embed = model.tgt_embed.cpu()
    nll_loss = nll_loss.to('cuda:1')
    cross_entropy_loss = cross_entropy_loss.to('cuda:1')
...
def train(args):
    ...
    while True:
        ...
        for src_sents, tgt_sents in data_iter(train_data, batch_size=args.batch_size):
            ...
            src_sents_var = to_input_variable(src_sents, vocab.src, cuda=args.cuda,device=args.device0)
            tgt_sents_var = to_input_variable(tgt_sents, vocab.tgt, cuda=args.cuda,device=args.device0)
            ...
            # print(scores)
            # if args.cuda:
            #     tgt_sents_var = tgt_sents_var.to(args.device0)
            tgt_sents_var0 = tgt_sents_var.to('cuda:1')
            del tgt_sents_var
            word_loss = cross_entropy_loss(scores.view(-1, scores.size(2)), tgt_sents_var0[1:].view(-1))
            loss = (word_loss / batch_size).to(args.device0)
            ...
            if train_iter % args.valid_niter == 0:
                ...
                dev_loss = evaluate_loss(args,model, dev_data, cross_entropy_loss)
                dev_ppl = np.exp(dev_loss)

                if args.valid_metric in ['bleu', 'word_acc', 'sent_acc']:
                    dev_hyps = decode(args,model, dev_data)
                    dev_hyps = [hyps[0] for hyps in dev_hyps]
                    print(dev_hyps[:3])
                    if args.valid_metric == 'bleu':
                        print(dev_data[0])
                        valid_metric = get_bleu([tgt for src, tgt in dev_data], dev_hyps, 'valid')
                    else:
...
def get_bleu(references, hypotheses, state):
    bleu_score = 0.0
...
def decode(args,model, data, verbose=False):
    """
    ...
    if type(data[0]) is tuple:
        for src_sent, tgt_sent in data:
            hyps = model.translate(args,src_sent)
...
def compute_lm_prob(args):
    ...
    for src_sent, tgt_sent in test_data:
        ...
        # (sent_len, batch_size)
        src_sents_var = to_input_variable(src_sents, model.vocab.src, cuda=args.cuda, is_test=True,device=args.device0)
        tgt_sents_var = to_input_variable(tgt_sents, model.vocab.tgt, cuda=args.cuda, is_test=True,device=args.device0)
...
def test(args):
    ...
    print('Sentence Level BLEU: %f, word level acc: %f, sentence level acc: %f' \
    % (bleu_score, word_acc, sent_acc), file=sys.stderr)

    if args.save_to_file:
        print('save decoding results to %s' % args.save_to_file, file=sys.stderr)
        with open(args.save_to_file, 'w') as f:
            for i in range(len(hypotheses)):
                f.write(str(i) + ': ref: ' + ' '.join(test_data_tgt[i][1:-1]) + '\n')
                f.write(str(i) + ': src: ' + ' '.join(hypotheses[i][0][1:-1]) + '\n')
